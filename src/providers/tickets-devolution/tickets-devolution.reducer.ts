...
import {
  getTicketFromCode as buildTicketFromCode,
  getSorteoFromCode,
  getLoteriaFromCode,
...
export const reducer = (
  ...
  switch (action.type) {
    case ADD_LOTTERY_TICKET: //It receives the entire ticket code
      ...
      const newTicket = buildTicketFromCode(
        action.codigo,
        ticketsCounter,
        state.leerXFracciones
      );
      ...
      let existingTicket =
        state.ticketsCollection.byId[
          state.leerXFracciones ? action.codigo : action.codigo.substr(0, action.codigo.length - 2)
        ];
      ...
      if (existingTicket) {
        ...
        if (
          !state.leerXFracciones &&
          +newTicket.fraccion < +existingTicket.fraccion
        ) {
          return state;
        }

        //another fraction from the same ticket
        if (state.leerXFracciones) {
        ...
        existingTicketToRemoveFromCounterIndex = state.ticketsCounterCollection.byId[
          existingTicket.cantidadFracciones
        ].tickets.indexOf(existingTicket.codigo);
        existingFractionTickets =
      ...
      if (existingTicket && existingCounter) {
        ...
        existingTicketIndex = existingCounter.tickets.indexOf(
          existingTicket.codigo
        );
      }
      ...
      return {
        ...
        ticketsCounterCollection: {
            ...
            ? {
                ...
                [existingCounter.codigo]: {
                  ...
                  tickets: [
                    ...state.ticketsCounterCollection.byId[
                      existingCounter.codigo
                    ].tickets.slice(0, existingTicketIndex),
                    ...state.ticketsCounterCollection.byId[
                      existingCounter.codigo
                    ].tickets.slice(
            ...
            ? //Ticket already exists
              {
                ...
                [existingTicket.cantidadFracciones]: {
                  ...
                  tickets: [
                    ...existingFractionTickets.slice(
                      0,
                      existingTicketToRemoveFromCounterIndex
                    ),
                    ...existingFractionTickets.slice(
                      existingTicketToRemoveFromCounterIndex + 1,
                      existingFractionTickets.length
                    ),
                  ],
                },
              }
          ...
          allIds: existingCounter
            ? state.ticketsCounterCollection.allIds
            : [
                ...state.ticketsCounterCollection.allIds,
                newTicket.cantidadFracciones,
              ],
        ...
        ticketsCollection: {
          byId: {
            ...state.ticketsCollection.byId,
            [newTicket.codigo]:
              state.leerXFracciones && existingTicket
                ? existingTicket
                : newTicket,
          },
          allIds: existingTicket
            ? state.ticketsCollection.allIds
            : [...state.ticketsCollection.allIds, newTicket.codigo],
        },
      };
    ...
    case UPDATE_TICKET_CANTIDAD: {
      const ticketRemoveIndx = state.ticketsCounterCollection.byId[
        action.ticket.cantidadFracciones
      ].tickets.indexOf(action.ticket.codigo);
      const ticketsLength =
          ...
          byId: {
            ...
            [action.ticket.cantidadFracciones]: {
              ...
              tickets: [
                ...state.ticketsCounterCollection.byId[
                  action.ticket.cantidadFracciones
                ].tickets.slice(0, ticketRemoveIndx),
                ...state.ticketsCounterCollection.byId[
                  action.ticket.cantidadFracciones
                ].tickets.slice(ticketRemoveIndx + 1, ticketsLength),
              ],
            },
            [action.newCounter]: {
    ...
    case REMOVE_TICKET: {
      const ticketCounterRemoveIndx = state.ticketsCounterCollection.byId[
        action.ticket.cantidadFracciones
      ].tickets.indexOf(action.ticket.codigo);
      const ticketsCounterLength =
        state.ticketsCounterCollection.byId[action.ticket.cantidadFracciones]
          .tickets.length;

      const ticketRemoveIndx = state.ticketsCollection.allIds.indexOf(
        action.ticket.codigo
      );
      const ticketsLength = state.ticketsCollection.allIds.length;

      let {
        [action.ticket.codigo]: removedItems,
        ...updatedTicketsCollection
      } = state.ticketsCollection.byId;
      return {
        ...state,
        ticketsCounterCollection: {
          ...
          byId: {
            ...
            [action.ticket.cantidadFracciones]: {
              ...
              tickets: [
                ...state.ticketsCounterCollection.byId[
                  action.ticket.cantidadFracciones
                ].tickets.slice(0, ticketCounterRemoveIndx),
                ...state.ticketsCounterCollection.byId[
                  action.ticket.cantidadFracciones
                ].tickets.slice(
                  ticketCounterRemoveIndx + 1,
                  ticketsCounterLength
                ),
              ],
            },
          },
        ...
        ticketsCollection: {
          ...
          allIds: [
            ...
            ...state.ticketsCollection.allIds.slice(
              ticketRemoveIndx + 1,
              ticketsLength
            ),
          ],
    ...
    case RESET_COUNTER:
      ...
      for (let [ticketCode, ticket] of Object.entries(
        state.ticketsCollection.byId
      )) {
